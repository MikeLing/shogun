		...
		while (sz == blocksize)
		{
			...
			for (size_t i=0; i<sz; i++)
			{
				if (dummy[i]=='\n' || (i==sz-1 && sz<blocksize))
				{
					...
					max_string_length=CMath::max(max_string_length, index_t(len+overflow_len));
					...
					if (remap_to_bin)
					{
						for (index_t j=0; j<overflow_len; j++)
...
template<class ST> void CStringFeatures<ST>::embed_features(int32_t p_order)
{
	...
	ST mask=0;
	for (index_t i=0; i<p_order*max_val; i++)
		mask= (mask<<1) | ((ST) 1);
...
template<class ST> void CStringFeatures<ST>::compute_symbol_mask_table(int64_t max_val)
{
	...
	uint64_t mask=0;
	for (index_t i=0; i< (int64_t) max_val; i++)
		mask=(mask<<1) | 1;
